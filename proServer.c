...
#define PORT 9000
#define BUFSIZE 100
char hi[BUFSIZE] = "안녕하세요. 만나서 반가워요.";
char name[BUFSIZE] = "내 이름은 컴퓨터야.";
char age[BUFSIZE] = "나는 다섯 살이야.";
char none[BUFSIZE] = "무슨 뜻인지 모르겠어요.";
	...
	while(1){ //무한 루프
		...
		while(1){
			...
			if(strncasecmp(rcvBuffer, "quit", 4) == 0 || strncasecmp(rcvBuffer,"kill server",11) == 0)
				break;
			else if(strncasecmp(rcvBuffer, "안녕", 2) == 0)
			...
			{
				sprintf(sndBuffer, "string length is %d", strlen(rcvBuffer)-7);
